...
use zeroclaw_plugins::egress::{EgressDecisionInput, EgressPolicy};
...
fn policy(hosts: &[&str], allow_private: &[&str]) -> EgressPolicy {
    EgressPolicy::fixed(EgressDecisionInput {
        hosts: hosts.iter().map(|h| (*h).to_string()).collect(),
        allow_private: allow_private.iter().map(|h| (*h).to_string()).collect(),
    })
}

/// Run the fixture's `execute` under `egress` and return its report line.
async fn probe(url: &str, follow: bool, egress: Option<EgressPolicy>) -> String {
