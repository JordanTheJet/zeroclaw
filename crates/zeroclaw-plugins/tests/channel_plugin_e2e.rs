...
#[tokio::test]
async fn poll_ingress_denies_an_uncanonicalizable_sender_under_a_wildcard_policy() {
    // A wildcard peer allowlist is the weakest operator policy the host will
    // honor. Even there, the guest-asserted sender has to canonicalize: an
    // empty or control-bearing identity is not a sender, and it must never
    // reach the shared dispatcher or the cancellation key.
    let channel = channel("main")
        .await
        .with_sender_authorizer(Arc::new(|_| true));
    let inbound = channel.inbound();
    ...
    inbound.enqueue(message("empty", ""));
    inbound.enqueue(message("blank", "   "));
    inbound.enqueue(message("control", "a\u{0}b"));
    inbound.enqueue(message("padded", "  alice  "));

    let (tx, mut receiver) = tokio::sync::mpsc::channel(4);
    let listener = zeroclaw_spawn::spawn!(async move { channel.listen(tx).await });
    let delivered = tokio::time::timeout(Duration::from_secs(5), receiver.recv())
        .await
        .expect("the canonicalizable sender arrives")
        .expect("listener remains connected");
    assert_eq!(
        delivered.id, "padded",
        "the three uncanonicalizable senders must be dropped before delivery"
    );
    assert_eq!(
        delivered.sender, "alice",
        "the delivered sender is the canonicalized value"
    );
    assert!(receiver.try_recv().is_err());
