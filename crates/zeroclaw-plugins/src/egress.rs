...
use zeroclaw_api::plugin_egress::{
    OutboundHostPattern, is_valid_secret_property_ref, is_valid_tls_profile_name,
};
...
/// Validated reference to a top-level, schema-designated instance secret.
///
/// This is intentionally not a URI and carries no plugin identity. The type
/// validates only the portable property-name syntax. The request's host-issued
/// [`PluginInstanceScope`] supplies identity, and an adapter must verify the
/// manifest schema's secret designation while resolving that same instance's
/// config at point of use. General cross-service secret references are outside
/// this type's contract.
#[derive(Clone, Debug, PartialEq, Eq, Hash)]
pub struct SecretPropertyRef(String);

impl SecretPropertyRef {
    /// Validate a top-level secret property name.
    ///
    /// # Errors
    ///
    /// Returns [`EgressError::InvalidSecretReference`] for empty, oversized,
    /// or non-portable property names.
    pub fn new(property: impl Into<String>) -> Result<Self, EgressError> {
        let property = property.into();
        if !is_valid_secret_property_ref(&property) {
            return Err(EgressError::InvalidSecretReference(property));
        }
        Ok(Self(property))
    }

    /// Property name passed to the existing instance-scoped secret service.
    #[must_use]
...
mod tests {
    ...
    fn tls_profiles_hold_only_same_instance_secret_property_references() {
        let profile = TlsProfile::new(
            ...
            true,
            Some(SecretPropertyRef::new("corporate_ca_pem").unwrap()),
