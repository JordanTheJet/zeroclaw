...
//! with the instance's [`EgressHostService`](crate::egress::EgressHostService),
//! so a socket reaches exactly the hosts the operator granted in
//! `egress_hosts`, under the same address-class rules and connection budget as
//! `wasi:http`. Plaintext is not a separate grant (ADR-014).
...
mod tests {
    ...
    impl TestPki {
        ...
        fn client_config(&self, profile: Option<&TlsProfile>) -> Arc<rustls::ClientConfig> {
            build_tls_client_config(profile, &rustls::RootCertStore::empty(), |reference| {
    ...
    /// A service granting `hosts`, each allowed to resolve to loopback (the
    /// test servers bind 127.0.0.1), with `profiles` attached.
    fn service(
        hosts: impl IntoIterator<Item = &'static str>,
        profiles: impl IntoIterator<Item = TlsProfile>,
        max_connections: usize,
    ) -> EgressHostService {
        let hosts = hosts.into_iter().map(str::to_string).collect::<Vec<_>>();
        ...
        EgressHostService::with_private_connection_accounting(EgressPolicyResolver::new(
            move |_| {
                EgressPolicy::new(&hosts, &hosts, &[], max_connections)?
                    .with_tls_profiles(profiles.clone())
            },
        ))
    }
    ...
    fn authorize(
        ...
        let mut request = EgressRequest::new(scope(binding), transport, host, address.port())
    ...
    async fn plaintext_dials_only_the_pinned_address_and_round_trips() {
        ...
        let service = service(["service.example"], [], 2);
    ...
    async fn connection_resource_retains_and_releases_the_shared_lease() {
        let address = start_plain_echo().await;
        let service = service(["service.example"], [], 1);
        let request = || {
            EgressRequest::new(
                scope("main"),
                EgressTransport::Tcp,
    ...
    async fn direct_tls_rejects_an_untrusted_server() {
        ...
        let config = build_tls_client_config(None, &rustls::RootCertStore::empty(), |_| {
    ...
    async fn failed_starttls_upgrade_is_terminal_without_plaintext_fallback() {
        ...
        connection
            .commands
            .send(ActorCommand::UpgradeTls {
                config: build_tls_client_config(None, &rustls::RootCertStore::empty(), |_| {
