...
/// Compile or deserialize the exact component bytes admitted by the host. With
/// a JIT backend present the admitted `.wasm` payload is compiled on load; in
/// runtime-only builds it is a precompiled `.cwasm` deserialized directly.
