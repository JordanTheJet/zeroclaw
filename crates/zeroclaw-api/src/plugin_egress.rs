...
/// Whether `property` can name one top-level plugin secret-schema property.
#[must_use]
pub fn is_valid_secret_property_ref(property: &str) -> bool {
...
mod tests {
    ...
    #[test]
    fn profile_and_secret_identifiers_reject_path_or_uri_syntax() {
        assert!(is_valid_tls_profile_name("corporate-mtls"));
        assert!(!is_valid_tls_profile_name("Corporate"));
        assert!(is_valid_secret_property_ref("client_key.pem"));
        for invalid in ["plugin://other/key", "../key", "key:value", "key/value"] {
            assert!(!is_valid_secret_property_ref(invalid), "{invalid:?}");
        }
    }
}
