...
mod tests {
    use super::*;
    use crate::ceremony::{PresenceAttestation, run_genesis};
    ...
    ///
    /// Compiled only under `cfg(test)`. It reports [`PresenceClass::Terminal`]
    /// because that is the strongest class this phase defines, and no adapter
    /// may name a class it cannot achieve.
    ...
    impl ScriptedPresence {
        fn affirming(identity: &str) -> Self {
            Self {
        ...
        fn affirming_without_identity() -> Self {
            Self {
                outcome: Ok(None),
                seen: Mutex::new(Vec::new()),
            }
                ...
                Ok(identity) => Ok(PresenceAttestation::for_test(
                    ...
                    match (identity, &request.operator_prompt) {
                        (Some(value), Some(_)) => Some(
                            FirstOperatorIdentity::new(value.clone())
    ...
    fn genesis_instance(tmp: &tempfile::TempDir) -> (PathBuf, ControlPaths, GenesisRecord) {
        let root = install_root(tmp);
        run_genesis(
            &root,
            &ScriptedPresence::affirming("jordan"),
            &crate::ceremony::GenesisRequest {
        ...
        let record = classify(&paths, &paths.key_source())
            .record()
            .expect("managed")
            .clone();
        (root, paths, record)
    ...
    fn key_for(paths: &ControlPaths, record: &GenesisRecord) -> ApprovalAuditKey {
        ApprovalAuditKey::derive(&paths.key_source(), record.trust_epoch).expect("derive")
    }

    fn operator(name: &str) -> OperatorIdentity {
    ...
    ///
    /// Nothing in this phase can actually discharge those proofs; this is the
    /// only way a test can exercise the eligible branch at all, and that is
    /// itself the honest statement of where the phase stands.
    fn isolated_requester(subject: &str) -> RequesterContext {
    ...
    fn agent_domain() -> std::collections::BTreeSet<String> {
        crate::client_registry::PROPOSAL_DOMAINS_V1
            .iter()
            .map(|d| (*d).to_string())
            .collect()
    ...
    fn unprovable_requester(subject: &str) -> RequesterContext {
