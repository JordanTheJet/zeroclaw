...
/// Per-entry email allowlist comparison: domain-class matching used by the
/// `gmail_push` and `email` channels (and their plugin mirrors), so the
/// address-vs-domain rule lives in exactly one place.
///
/// - `"@example.com"` (leading `@`) admits any address in that domain.
/// - `"user@example.com"` (contains `@`) is a full case-insensitive address.
/// - `"example.com"` (bare domain) admits any address in that domain.
///
/// Feed it to [`is_user_allowed_by`] so the wildcard/empty semantics stay
...
mod tests {
    ...
    #[test]
    fn email_match_domain_and_full_address() {
        // Leading-@ and bare domain both admit the whole domain; a full address
        // matches case-insensitively; a different domain is denied.
        assert!(email_match("@example.com", "anyone@Example.com"));
        assert!(email_match("example.com", "someone@example.com"));
        assert!(email_match("boss@corp.io", "BOSS@corp.io"));
        assert!(!email_match("@example.com", "user@evil.com"));
    }

    #[test]
    fn email_match_via_is_user_allowed_by() {
        // The wrapper keeps wildcard/empty identical to other channels.
        let list = vec!["@example.com".to_string(), "boss@corp.io".to_string()];
        assert!(is_user_allowed_by(&list, "anyone@example.com", email_match));
        assert!(!is_user_allowed_by(&list, "stranger@evil.com", email_match));
        assert!(!is_user_allowed_by(&[], "anyone@example.com", email_match));
