...
cli-quickstart-liveness-checking = Checking that the chosen model answers before saving…
cli-quickstart-liveness-ok = ✓ {$model} answered via {$provider}.
cli-quickstart-liveness-auth = {$provider} rejected the credential: {$detail}
cli-quickstart-liveness-auth-abort = Fix the API key and run quickstart again, or pass --no-verify to skip this check.
cli-quickstart-liveness-model = {$provider} does not serve the model {$model}: {$detail}
cli-quickstart-liveness-model-abort = Check the model id and run quickstart again, or pass --no-verify to skip this check.
cli-quickstart-liveness-warn = ⚠ Could not verify the model via {$provider} ({$detail}). Continuing — if first contact fails, check connectivity and run `zeroclaw doctor`.
