...
cli-plugin-egress-invalid-grant = {$name}: its config entry grants {$hosts}, which is not a valid destination pattern — the runtime rejects the entire allowlist, so every request is denied until it is fixed. Replace the grant with: {$command}
cli-plugin-egress-invalid-grant-legacy = {$name}: its config entry grants {$hosts}, which is not a valid destination pattern — the runtime rejects the entire allowlist, so every request is denied until it is fixed. Its config row still uses the pre-1.0 key format, so migrate the row, then replace the grant:
