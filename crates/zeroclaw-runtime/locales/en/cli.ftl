...
# ── Zerona agent creator ──
cli-onboard-legacy-flags = error: legacy `zeroclaw onboard` flags no longer apply. Use `zeroclaw quickstart` to create a new agent, or `zeroclaw config set <path>=<value>` for headless updates.
cli-zerona-tty-required = `zeroclaw onboard` requires an interactive terminal.
cli-zerona-no-provider = No capability-restricted model provider is ready. Configure one with `zeroclaw quickstart`, then run `zeroclaw onboard` again.
cli-zerona-provider-prompt = Choose the model provider Zerona and the new agent will use
cli-zerona-intro = Zerona will help you create one contained agent using {$provider}. Describe who the agent should help and how it should behave. Type `quit` at any time to leave without applying a proposal.
cli-zerona-turn-prompt = You:
cli-zerona-review-prompt = Type `apply`, `revise`, or `cancel`:
cli-zerona-review-invalid = Choose exactly `apply`, `revise`, or `cancel`.
cli-zerona-revision-prompt = What should Zerona change?
cli-zerona-cancelled = Zerona exited without applying the current proposal.
cli-zerona-complete = Created agent `{$alias}`. Start it with `zeroclaw agent --agent {$alias}`.

cli-zerona-preview-title = Proposed agent
cli-zerona-preview-agent = Agent: {$value}
cli-zerona-preview-provider = Model provider: {$value}
cli-zerona-preview-model = Model: {$value}
cli-zerona-preview-risk = Risk profile: {$value}
cli-zerona-preview-runtime = Runtime profile: {$value}
cli-zerona-preview-memory = Memory: {$value}
cli-zerona-preview-workspace = Workspace to create: {$path}
cli-zerona-preview-workspace-not-created = Workspace: not created because this proposal contains no personality files
cli-zerona-preview-config = Config file to update: {$path}
cli-zerona-preview-config-write = Config path to update: {$path}
cli-zerona-preview-personality = Personality files
cli-zerona-preview-personality-empty = No personality files will be written.
cli-zerona-preview-file-heading = ┌─ {$filename}
cli-zerona-preview-file-destination = │ destination: {$path}
cli-zerona-preview-file-overwrite-warning = │ an existing file at this destination may be replaced during apply
cli-zerona-preview-file-end = └─ end {$filename}
cli-zerona-preview-posture = Effective security posture
cli-zerona-preview-autonomy = Autonomy: {$value}
cli-zerona-preview-workspace-only = Workspace-only filesystem: {$value}
cli-zerona-preview-high-risk-blocked = High-risk commands blocked: {$value}
cli-zerona-preview-filesystem-unconfined = Filesystem unconfined: {$value}
cli-zerona-preview-allowed-root = Additional filesystem root: {$path}
cli-zerona-preview-allowed-root-read-only = Additional read-only root: {$path}
cli-zerona-preview-allowed-root-write-only = Additional write-only root: {$path}

cli-zerona-error-input-too-large = That message is too large for the onboarding terminal.
cli-zerona-error-config-busy = config.toml changed repeatedly while Zerona was loading it. Try again after the other editor finishes.
cli-zerona-error-current-schema-required = Zerona requires config.toml to be saved at the current schema version before it can create an agent. Run `zeroclaw config migrate`, review the migrated file, then restart onboarding.
cli-zerona-error-healthy-config-required = Zerona cannot create an agent while config.toml contains sections that were dropped during resilient loading. Repair or migrate the reported config sections, then restart onboarding.
cli-zerona-error-session-config-changed = config.toml changed while Zerona was running. Restart onboarding so no prior conversation is sent through a changed provider or policy configuration.
cli-zerona-error-verification = The saved agent did not match the approved proposal. Inspect config.toml before running it.
cli-zerona-error-partial-apply = ZeroClaw could not confirm whether the agent configuration committed. The workspace was preserved; inspect config.toml and the agent workspace before retrying.
cli-zerona-error-turn-limit = Zerona reached the 48-turn session limit without an applied proposal.
cli-zerona-error-operator-input = That input was not sent to the model because it contains a credential shape, URI, raw configuration, or unsafe terminal text.
cli-zerona-error-provider-unavailable = The selected provider is no longer available for this restricted session.
cli-zerona-error-provider-call = Zerona could not complete the provider request. No proposal was applied.
cli-zerona-error-provider-returned-tool-call = The provider returned a tool call even though Zerona exposes no tools. The response was rejected.
cli-zerona-error-model-output = Zerona returned credential-shaped, URI, raw-configuration, or unsafe terminal text. The response was rejected.
cli-zerona-error-proposal = Zerona's proposal was rejected at {$field}.
cli-zerona-error-credential-content = {$field} contains credential-shaped content and cannot cross this boundary.
cli-zerona-error-config-content = {$field} contains a URI or raw configuration content that Zerona cannot accept.
cli-zerona-error-terminal-control = {$field} contains unsupported terminal control characters.
cli-zerona-error-bidi-control = {$field} contains bidirectional control characters.
cli-zerona-error-proposal-json = Zerona returned an invalid proposal object.
cli-zerona-error-agent-alias = The proposed agent alias is invalid.
cli-zerona-error-agent-alias-reserved = The proposed agent alias is reserved.
cli-zerona-error-agent-alias-exists = The proposed agent alias already exists.
cli-zerona-error-risk-unavailable = The selected contained risk preset is unavailable or was locally modified.
cli-zerona-error-runtime-unavailable = The selected bounded runtime preset is unavailable or was locally modified.
cli-zerona-error-personality-filename = {$field} is not an allowed Zerona-authored personality filename.
cli-zerona-error-personality-duplicate = {$field} repeats a personality filename.
cli-zerona-error-personality-too-large = {$field} exceeds the {$limit}-character limit.
cli-zerona-error-personality-too-many = The proposal contains more than {$limit} personality files.
cli-zerona-error-uncontained-posture = Zerona cannot create an agent whose effective posture is uncontained.
cli-zerona-error-profile-drift = A profile changed after preview. Zerona must produce a new proposal against current config.
cli-zerona-error-provider-target-drift = The selected provider target changed after preview. Zerona must produce a new proposal against current config.
cli-zerona-error-provider-not-configured = Model provider {$provider} is not configured.
cli-zerona-error-provider-capability-denied = Model provider {$provider} cannot run in Zerona's capability-restricted session.
cli-zerona-error-provider-host-process = Model provider {$provider} can execute a host process and is not available to Zerona.
cli-zerona-error-provider-missing-model = Model provider {$provider} has no configured model.
