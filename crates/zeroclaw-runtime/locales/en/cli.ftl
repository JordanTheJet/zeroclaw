...
cli-plugin-toggle-novel-channel = '{$id}' is a novel plugin channel and has no typed channel config to toggle. Its current lifecycle follows `plugins.enabled`; per-plugin enable/disable is not available yet.
cli-plugin-source-conflict = plugin conflict
