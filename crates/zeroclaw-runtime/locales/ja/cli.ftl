...
cli-plugin-egress-invalid-grant = {$name}: 設定エントリが {$hosts} を許可していますが、これは有効な送信先パターンではありません — ランタイムは許可リスト全体を拒否するため、修正するまですべてのリクエストが拒否されます。次のコマンドで許可を置き換えてください: {$command}
cli-plugin-egress-invalid-grant-legacy = {$name}: 設定エントリが {$hosts} を許可していますが、これは有効な送信先パターンではありません — ランタイムは許可リスト全体を拒否するため、修正するまですべてのリクエストが拒否されます。設定行が 1.0 より前のキー形式のままなので、行を移行してから許可を置き換えてください:
