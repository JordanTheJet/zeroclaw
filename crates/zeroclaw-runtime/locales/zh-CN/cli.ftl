...
cli-plugin-egress-invalid-grant = {$name}：其配置条目授予了 {$hosts}，这不是有效的目标模式 — 运行时会拒绝整个允许列表，在修复之前所有请求都会被拒绝。请用以下命令替换授权：{$command}
cli-plugin-egress-invalid-grant-legacy = {$name}：其配置条目授予了 {$hosts}，这不是有效的目标模式 — 运行时会拒绝整个允许列表，在修复之前所有请求都会被拒绝。其配置行仍使用 1.0 之前的键格式，请先迁移该行，然后替换授权：
