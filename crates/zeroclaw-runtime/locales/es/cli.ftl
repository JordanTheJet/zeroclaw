...
cli-plugin-egress-invalid-grant = {$name}: su entrada de configuración concede {$hosts}, que no es un patrón de destino válido — el runtime rechaza toda la lista de permitidos, así que todas las solicitudes se deniegan hasta que se corrija. Reemplaza la concesión con: {$command}
cli-plugin-egress-invalid-grant-legacy = {$name}: su entrada de configuración concede {$hosts}, que no es un patrón de destino válido — el runtime rechaza toda la lista de permitidos, así que todas las solicitudes se deniegan hasta que se corrija. Su fila de configuración todavía usa el formato de clave anterior a 1.0, así que migra la fila y luego reemplaza la concesión:
