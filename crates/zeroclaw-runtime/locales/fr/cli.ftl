...
cli-plugin-egress-invalid-grant = {$name} : son entrée de configuration accorde {$hosts}, qui n'est pas un motif de destination valide — le runtime rejette toute la liste d'autorisation, donc chaque requête est refusée tant que ce n'est pas corrigé. Remplacez l'autorisation par : {$command}
cli-plugin-egress-invalid-grant-legacy = {$name} : son entrée de configuration accorde {$hosts}, qui n'est pas un motif de destination valide — le runtime rejette toute la liste d'autorisation, donc chaque requête est refusée tant que ce n'est pas corrigé. Sa ligne de configuration utilise encore le format de clé antérieur à 1.0 : migrez la ligne, puis remplacez l'autorisation.
