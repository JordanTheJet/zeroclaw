...
pub fn resolve_model_provider_type(type_key: &str) -> Option<(&'static str, bool)> {
    if let Ok(provider) = type_key.parse::<zeroclaw_providers::auth::AuthProvider>() {
...
pub fn field_shape(section: FieldSection, type_key: &str) -> Vec<FieldDescriptor> {
    ...
    if matches!(section, FieldSection::ModelProvider) && section_path == "providers.models.openai" {
        out.push(openai_auth_mode_descriptor(codex_auth_preselected));
    }
    if matches!(section, FieldSection::ModelProvider)
        && section_path == "providers.models.anthropic"
    {
        out.push(anthropic_auth_mode_descriptor());
    }
    out.sort_by_key(|d| {
        essentials
            .iter()
...
const QUICKSTART_OPENAI_AUTH_MODE_API_KEY: &str = "api_key";
const QUICKSTART_OPENAI_AUTH_MODE_CODEX: &str = "codex";
const QUICKSTART_ANTHROPIC_AUTH_MODE_API_KEY: &str = "api_key";
const QUICKSTART_ANTHROPIC_AUTH_MODE_SETUP_TOKEN: &str = "setup_token";

fn openai_auth_mode_descriptor(codex_auth_preselected: bool) -> FieldDescriptor {
    FieldDescriptor {
        key: QUICKSTART_AUTH_MODE_FIELD.to_string(),
        label: crate::i18n::get_required_cli_string("cli-quickstart-openai-auth-mode-label"),
        help: crate::i18n::get_required_cli_string("cli-quickstart-openai-auth-mode-help"),
        kind: zeroclaw_config::traits::PropKind::Enum,
        is_secret: false,
        enum_variants: Some(vec![
            QUICKSTART_OPENAI_AUTH_MODE_API_KEY.to_string(),
            QUICKSTART_OPENAI_AUTH_MODE_CODEX.to_string(),
        ]),
        required: true,
        default: Some(
            if codex_auth_preselected {
                QUICKSTART_OPENAI_AUTH_MODE_CODEX
            } else {
                QUICKSTART_OPENAI_AUTH_MODE_API_KEY
            }
            .to_string(),
        ),
    }
...
fn anthropic_auth_mode_descriptor() -> FieldDescriptor {
    FieldDescriptor {
        key: QUICKSTART_AUTH_MODE_FIELD.to_string(),
        label: crate::i18n::get_required_cli_string("cli-quickstart-anthropic-auth-mode-label"),
        help: crate::i18n::get_required_cli_string("cli-quickstart-anthropic-auth-mode-help"),
        kind: zeroclaw_config::traits::PropKind::Enum,
        is_secret: false,
        enum_variants: Some(vec![
            QUICKSTART_ANTHROPIC_AUTH_MODE_API_KEY.to_string(),
            QUICKSTART_ANTHROPIC_AUTH_MODE_SETUP_TOKEN.to_string(),
        ]),
        required: true,
        default: Some(QUICKSTART_ANTHROPIC_AUTH_MODE_API_KEY.to_string()),
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum OpenAiQuickstartAuthMode {
    ApiKey,
    Codex,
}

fn parse_openai_quickstart_auth_mode(raw: &str) -> Option<OpenAiQuickstartAuthMode> {
    match raw.trim().to_ascii_lowercase().as_str() {
        "" | QUICKSTART_OPENAI_AUTH_MODE_API_KEY => Some(OpenAiQuickstartAuthMode::ApiKey),
        QUICKSTART_OPENAI_AUTH_MODE_CODEX
        | "codex_subscription"
        | "openai-codex"
        | "openai_codex" => Some(OpenAiQuickstartAuthMode::Codex),
        _ => None,
    }
}

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum AnthropicQuickstartAuthMode {
    ApiKey,
    SetupToken,
}

fn parse_anthropic_quickstart_auth_mode(raw: &str) -> Option<AnthropicQuickstartAuthMode> {
    match raw.trim().to_ascii_lowercase().as_str() {
        "" | QUICKSTART_ANTHROPIC_AUTH_MODE_API_KEY | "key" => {
            Some(AnthropicQuickstartAuthMode::ApiKey)
        }
        QUICKSTART_ANTHROPIC_AUTH_MODE_SETUP_TOKEN | "setup-token" | "claude_setup_token" => {
            Some(AnthropicQuickstartAuthMode::SetupToken)
        }
        _ => None,
    }
}
    ...
    match choice {
        ...
        SelectorChoice::Fresh(choice) => {
            ...
            let auth_mode = if provider_type == "openai" {
                match choice.fields.get(QUICKSTART_AUTH_MODE_FIELD) {
                    Some(value) => match parse_openai_quickstart_auth_mode(value) {
                        Some(mode) => mode,
                        None => {
                            errors.push(QuickstartError::for_surface(
                                ctx,
                                ...
                                format!(
                                    "unknown OpenAI auth mode `{}` — pick `api_key` or `codex`",
                                    value.trim()
                                ),
                                "cli-quickstart-error-unknown-openai-auth-mode",
                                &[("mode", value.trim())],
                            ));
                            return None;
                        }
                    },
                    None if codex_alias_requested => OpenAiQuickstartAuthMode::Codex,
                    None => OpenAiQuickstartAuthMode::ApiKey,
                }
            } else {
                OpenAiQuickstartAuthMode::ApiKey
            };
            if provider_type == "anthropic"
                && let Some(value) = choice.fields.get(QUICKSTART_AUTH_MODE_FIELD)
                && parse_anthropic_quickstart_auth_mode(value).is_none()
            {
                errors.push(QuickstartError::for_surface(
                    ctx,
                    QuickstartStep::ModelProvider,
                    QUICKSTART_AUTH_MODE_FIELD,
                    format!(
                        "unknown Anthropic auth mode `{}` — pick `api_key` or `setup_token`",
                        value.trim()
                    ),
            ...
            let codex_auth =
                provider_type == "openai" && matches!(auth_mode, OpenAiQuickstartAuthMode::Codex);
