...
/// An instance-local property name resolved by the secret service only when
/// the job is delivered. It contains no secret material.
#[derive(Clone, Debug, Eq, Hash, PartialEq)]
pub struct SecretPropertyRef(String);

impl SecretPropertyRef {
    /// Validate a manifest property name for durable reference storage.
    pub fn new(property: impl Into<String>) -> Result<Self> {
        let property = property.into();
        if !is_valid_secret_property_ref(&property) {
            bail!("invalid plugin secret property reference");
        }
        Ok(Self(property))
    }
...
// The portable top-level property grammar shared by plugin host services.
fn is_valid_secret_property_ref(property: &str) -> bool {
    (1..=128).contains(&property.len())
        && property
            .bytes()
...
mod tests {
    ...
    fn envelope(label: &str) -> PluginOutboxEnvelope {
        PluginOutboxEnvelope::new(
            ...
            vec![SecretPropertyRef::new("credential").unwrap()],
        )
        .unwrap()
    }
    ...
    fn oversized_payload_and_duplicate_secret_references_are_rejected() {
        assert!(SecretPropertyRef::new("client_key.pem").is_ok());
        for invalid in ["../key", "key/value", "key:value", "plugin://other/key"] {
            assert!(SecretPropertyRef::new(invalid).is_err(), "{invalid:?}");
        }
        ...
        assert!(
            PluginOutboxEnvelope::new(
                ...
                vec![
                    SecretPropertyRef::new("token").unwrap(),
                    SecretPropertyRef::new("token").unwrap(),
                ],
            )
            .is_err()
