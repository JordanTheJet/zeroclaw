...
pub(crate) async fn gate_tool_approval(
    ...
    if operator_only {
        if ctx.approval.is_none() {
            let denied =
                crate::i18n::get_required_cli_string("cli-operator-tool-approval-required");
        ...
        // A tool's structural boundary outranks profile and session grants.
        ...
        && approval_requirement == ApprovalRequirement::Prompt
    {
        ...
        let request = ApprovalRequest {
            ...
            arguments: loggable_args_value(tool, tool_args),
                ...
                if operator_only && !ch.is_operator_approval_surface() {
                    ...
                    let response = if let Some(cancel) = ctx.cancellation_token {
                        tokio::select! {
                            ...
                            response = request_approval => response,
                        }
                    } else {
...
mod cancellation_tests {
    ...
    async fn cancelling_turn_drops_pending_channel_approval() {
        ...
        let mut arguments = serde_json::json!({"command": "sleep 60"});
