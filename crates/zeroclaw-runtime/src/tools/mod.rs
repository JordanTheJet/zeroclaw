...
fn all_tools_with_runtime_on_thread(
    ...
    if root_config.web_search.enabled {
        ...
        // blocked. The delegate's sub-agent loops too, so the limiter stays on
        // the raw tool it is handed.
        let raw_web_search: Arc<dyn Tool> = Arc::new(RateLimitedTool::new(
