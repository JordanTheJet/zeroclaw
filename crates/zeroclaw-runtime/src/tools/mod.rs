...
fn resolved_plugin_egress_policy(
    ...
    use zeroclaw_plugins::egress::{
        EgressError, EgressPolicy, SecretPropertyRef, TlsClientIdentity, TlsProfile, TlsProfileName,
    };
        ...
        .map(|profile| {
            ...
            let client_identity = match (
                ...
                (Some(certificate), Some(private_key)) => Some(TlsClientIdentity::new(
                    SecretPropertyRef::new(certificate)?,
                    SecretPropertyRef::new(private_key)?,
                )),
                (None, None) => None,
                _ => {
...
fn plugin_host_services(
    ...
    zeroclaw_plugins::services::PluginHostServices::new(config, egress)
}
