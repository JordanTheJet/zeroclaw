...
impl RpcDispatcher {
    ...
    async fn handle_session_prompt(&self, params: &Value) -> RpcResult {
        ...
        // The queue wait was unbounded: the incarnation authorized above may
        // have been replaced under the same id. Bind this prompt to the exact
        // record present now, under the permit, or refuse it.
    ...
    async fn handle_session_configure(&self, params: &Value) -> RpcResult {
        ...
        // The authorized record carries the live generation every gated
        // setter below is bound to. If the session is replaced while we wait
        // for the lock, the re-validation below detects the mismatch (and a
        // changed owner) and rejects the stale configure.
        let Some(session_generation) = authorized.as_ref().and_then(|r| r.live_generation) else {
            return Err(rpc_err(SESSION_NOT_FOUND, "Session not found"));
        };
