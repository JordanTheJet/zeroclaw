...
`plugins-wit-v0` for the core interfaces, and the distinct
`plugins-wit-v0-sockets` for the `socket` interface so the host can link it
per-plugin. Everything graduates when the first stable Component Model
release ships.
