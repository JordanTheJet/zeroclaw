...
# The root binary's plugin modules are listed for the same reason: `mod
# plugins` and the plugin registry only compile under `plugins-wasm`, so the
# default-feature Test job cannot run their tests and this job is where they
# run. `src/main.rs` holds the plugin CLI itself.
