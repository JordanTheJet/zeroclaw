...
# Channel-crate changes must run the backend job: the shared inbound
# dispatcher carries the typed plugin event queue behind its own
# plugins-wasm feature, so this job can break with nothing under the plugin
# crate touched.
expect "channel crate dispatcher" "true" \
    "crates/zeroclaw-channels/src/orchestrator/mod.rs"
expect "channel crate plugin queue" "true" \
    "crates/zeroclaw-channels/src/plugin_event_dispatch.rs"
expect "mixed unrelated then channels" "true" \
    "web/src/pages/AgentChat.tsx" \
