...
4. **Memory allowlist**: the parent's sibling grants from `workspace.read_memory_from` plus the parent's own alias. Category restrictions remain exact and are never broadened by a SubAgent; the parent's own alias is always re-added so a SubAgent always sees the parent's rows. The current `SubAgentOverrides::allowed_agent_aliases` override is validated as a name subset, so it can only narrow which granted siblings are visible.
...
2. **Reachability**: the target agent must be in the caller's reachable set, resolved by `Config::reachable_delegate_target_configs`. The reachable set is the union of two per-agent sources on `[agents.<caller>]`, minus the caller itself:
   ...
   A bounded target inherits the caller's action/cost tracker with the effective ceilings tightened to the caller's values at every hop, so a caller's action budget binds its whole subtree no matter what the target's own profile allows. Spawned background and parallel workers restore the originating sender's rate-limit bucket before their first admission, so the budget stays per-sender across spawn boundaries instead of falling back to the unscoped bucket. The cost ceiling is carried the same way but is declared, not enforced, on delegated runs today: delegated sub-loops do not yet participate in cost tracking, so only the action budget is actively enforced inside the subtree. When the bounded target shares the caller's risk profile, it also inherits the caller's session workspace boundary. A bounded **cross-profile** target is allowed when it is reachable through the caller's delegate roster and `delegation_policy`; it runs under the target's resolved policy, while agentic tool availability is capped by the caller's tool registry.
...
Two approval and scoping rules apply to the granted instance. First, it is **delegate-only**: the task-management actions (`check_result`, `list_results`, `cancel_task`, `await_sessions`) are refused, because background task records live in a workspace-wide namespace without owner identity and a bounded child shares the delegating parent's workspace. Note the boundary precisely: this scopes the delegate API surface only. The underlying `delegate_results/*.json` records remain ordinary workspace files that any identity with workspace read access can read, and task records carry no owner binding; isolating them is a separate concern from the tool grant. Second, sub-agent loops have no operator approval route, so the tool fails closed on the target's own approval policy: if the target's risk profile would prompt for `delegate` (the supervised default, or the tool listed in `always_ask`), the nested call is refused with an error naming the profile. Nested delegation therefore requires the target profile to explicitly auto-approve `delegate` or run at full autonomy; a caller profile that would prompt keeps prompting in its own loop, unchanged.
