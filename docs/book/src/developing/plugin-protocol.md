...
vendor tunnels, polling clients, and other transports remain follow-ups. Both
inbound bridges submit through the host-owned typed event router, which resolves
the live declaration, exact agent owner, sender allowlist, and SOP interest for
every event and then admits it into the same control stage native channels use,
so a per-vendor listener follow-up plugs into one boundary rather than a second
ingress. The memory bridge
...
- `sender` is asserted by the guest and is not authenticated by the host: the
  component owns the vendor connection, so there is no independent host view of
  who spoke. The host canonicalizes the value before the allowlist compare and
  rejects one that is empty, whitespace only, control bearing, or over 512
  bytes, including under the `"*"` allow-all entry. `interruption-scope-id` is
  likewise guest asserted and is namespaced as `<binding>:<scope>`, so a
  component can group its own threads but cannot address the cancellation scope
  of another binding.
