...
| Android UI control (`android_screenshot`, `android_ui_read`, `android_action`, `android_launch`) | Config-gated, platform-gated (Android only), and Unix-only; depends on a companion APK reached over a Unix-domain socket. | Keep first-party while device control stays coupled to approval policy and autonomy gating. `android_action` injects real input, so its approval behavior must remain visible to the runtime rather than delegated to a plugin. |
