...
| `permissions` | no | Host services the code may reach: `http_client`, `config_read`, `socket_client`, `file_read`, `file_write`, `memory_read`, `memory_write` (`PluginPermission`). Only the first three are enforced today; the rest are accepted but inert. `socket_client` grants raw TCP (+TLS) only to globally routable destinations. |
