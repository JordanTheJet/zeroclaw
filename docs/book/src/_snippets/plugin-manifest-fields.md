...
| `permissions` | no | Host services the code may reach: `http_client`, `websocket_client`, `socket_client`, `config_read`, `file_read`, `file_write`, `memory_read`, `memory_write` (`PluginPermission`). HTTP, WebSocket, and config permissions are enforced today; raw sockets, filesystem, and memory permissions remain inert until their host imports land. Declaring `config_read` requires `config_schema`, and only tool/channel adapters currently deliver it. |
