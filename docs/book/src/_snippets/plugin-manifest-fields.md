...
| `permissions` | no | Host services the code may reach. `PluginPermission` is the canonical set. `config_read`, `http_client`, and `socket_client` are enforced today; other declarations can be accepted but inert until their host import is wired. Declaring `config_read` requires `config_schema`, and only tool/channel adapters currently deliver it. |
