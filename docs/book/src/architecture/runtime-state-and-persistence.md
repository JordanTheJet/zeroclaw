...
| SOP runs | `SopEngine` plus `SopRunStore` | `data/sop/runs.db` by default; none when persistence is disabled or initialization falls back to memory | SOP engine active/finished run caches | The durable store owns admission claims and persisted revisions; the engine restores active and terminal state on startup | Store initialization failure logs a warning and falls back to memory. Memory-backed audit records are not the run-lifecycle source of truth. Zerona intentionally uses a private in-memory system-SOP engine and keeps raw conversation state outside the run record. |
