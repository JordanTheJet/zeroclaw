...
| [ADR-016](./ADR-016-cron-extraction-and-transitional-exception.md) | proposed | Cron is extracted into its own crate, with a bounded exception for cron work landing before the extraction. |
...
ADR-016 remains proposed until `zeroclaw-cron` owns the scheduler, store, reconciliation, claim ownership, and precondition evaluation, `crates/zeroclaw-runtime/src/cron` is gone rather than re-exported, and the holding-crate contract no longer names cron as awaiting extraction.
