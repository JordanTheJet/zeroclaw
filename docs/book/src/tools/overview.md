...
| `web_search_tool` | Raw web search. **Not registered by default**; it is scoped inside `web_research` (see below). Provider is configurable: DuckDuckGo (default, no key), Brave, Tavily, SearXNG, Jina, Bocha, AnySearch, Serply, or Keenable (no key required; optional key lifts rate limits) |
