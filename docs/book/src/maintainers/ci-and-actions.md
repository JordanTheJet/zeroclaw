...
The runner inventories cover both `ci.yml` and `windows-tests.yml`, with workflow-qualified job identities so same-named jobs cannot hide one another. The `build` matrix does not wait on any detector, so its Linux, macOS, and Windows legs start immediately. Only the separate `windows-task-owner-recovery` job waits for the Blacksmith `path-changes` detector, because that detector's output selects whether it runs.
...
The required Windows build leg always runs its compile and voice-wake checks. The task-owner recovery tests run in their own `windows-task-owner-recovery` job, in parallel with that leg rather than after it, because they build the runtime unit-test harness: a test-profile compile that shares nothing with `cargo check` output and can cost much more than executing the focused tests. The job restores the Windows build leg's master-seeded cache under the same key (`shared-key: build`) and never writes a cache of its own. On PRs, `scripts/ci/windows_recovery_change_filter.sh` selects that job for control-plane or runtime module wiring, Cargo manifests and lockfiles, build scripts, toolchains, Cargo configuration, Actions, and its own CI wiring. Other PRs skip only this job, which the required gate treats as a non-fatal skip. Master pushes and merge-queue builds always execute it; missing change evidence defaults to execution.
