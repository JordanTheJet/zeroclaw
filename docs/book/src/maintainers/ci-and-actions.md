...
`fmt` checks formatting on a GitHub-hosted runner. The compile-heavy jobs name their Blacksmith runner label directly (see Build cache behavior below), so they declare no dependency on `fmt` and are created as soon as the run starts. `runs-on` resolves before a job exists, so reading that label from a `fmt` output previously put one GitHub-hosted queue wait in front of the whole Blacksmith fan-out. `CI Required Gate` needs `fmt` directly, so a formatting error still blocks merge, and `tests/architecture/ci_runner_labels.rs` fails the workspace suite if the repeated label or the cache-provider inputs drift apart. The four `Detect … changes` jobs likewise only run `git diff` and start concurrently. `CI Required Gate` aggregates the required results only. The advisory Windows jobs are deliberately absent from that gate and cannot block a PR. Branch protection pins the composite gate job. A PR cannot merge until this is green. The `master` push run keeps the same quality signal while seeding trusted Rust caches for later PR runs.
...
| `useblacksmith/rust-cache` (`v3.0.1`) | `ci.yml` (Blacksmith path of `./.github/actions/rust-cache`) | Cargo build/dependency caching on Blacksmith sticky disk; selected by the `fmt` job's `use_blacksmith` output (a checked-in constant) |
