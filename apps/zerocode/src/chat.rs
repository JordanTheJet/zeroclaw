...
impl Chat {
    ...
    pub(crate) fn start_entry_retry(&mut self) {
        ...
        // The retry worker is a throwaway pane that only creates the session,
        // but it must share this pane's claim registry so nothing it observes
        // can be claimed twice once its phase is adopted here.
        let inbound_request_claims = Arc::clone(&self.inbound_request_claims);
        ...
        self.entry_retry_attempt = Some(EntryRetryAttempt {
            ...
            worker: tokio::spawn(async move {
                let mut retry = Chat::new_with_claims(rpc, PaneKind::Chat, inbound_request_claims);
