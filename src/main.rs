...
Add pricing to the active provider profile or supply a catalog entry."
        ...
        Commands::Config { config_command } => match config_command {
            ...
            ConfigCommands::Patch { input, json } => {
                ...
                // The local command may create a missing keyed-list row (a
                // plugin's `plugins.entries` entry, for one); the remote writers
                // sharing this engine may not. See `KeyCreation::MapOrList`.
                let results = match zeroclaw_config::patch::apply_patch_ops_with(
                    &mut config,
                    &ops,
                    zeroclaw_config::patch::KeyCreation::MapOrList,
                ) {
                    Ok(results) => results,
                    Err(err) => {
                        let human = config_patch_human_message(&err);
...
"#;
    ...
    async fn the_absent_row_repair_creates_the_row_and_its_grant() {
        ...
        // Apply the patch as the handler does: through the shared engine with
        // the local command's list-row policy.
        let parsed: serde_json::Value = serde_json::from_str(&body).expect("a JSON Patch");
        let ops = zeroclaw_config::patch::parse_patch_ops(parsed).expect("the handler parses it");
        assert_eq!(ops.len(), 1, "{body}");
        assert_eq!(ops[0].op, "add", "{body}");
        zeroclaw_config::patch::apply_patch_ops_with(
            &mut config,
            &ops,
            zeroclaw_config::patch::KeyCreation::MapOrList,
        )
        .expect("the local command creates the row and writes the grant");
        Box::pin(config.save_dirty())
            .await
